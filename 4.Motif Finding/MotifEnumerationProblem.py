...
def is_pattern_with_at_most_d_mistakes_in_string(pattern, dna_string, d):
    ...
    return reduce(lambda x, y: x | y,
                  [hamming_distance(pattern, dna_string[i:i + k]) <= d for i in range(len(dna_string) - k + 1)])


def is_pattern_with_at_most_d__mistakes_in_dna(pattern, DNA, d):
    return reduce(lambda x, y: x & y,
                  [is_pattern_with_at_most_d_mistakes_in_string(pattern, DNA[i], d) for i in range(len(DNA))])
